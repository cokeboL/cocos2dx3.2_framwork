...
function toolkit.startLoading2(scene)
    ...
    if running then
        ...
        if not mask then
            ...
            text:setString("数据加载中...")
            text:setPosition(cc.p(director.center.x, director.center.y))
            --text:setVisible(false)
            ...
            running:addChild(mask, constant.zorderMax-2, constant.zorderMax-2)
        end
    end
end
